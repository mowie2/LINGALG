...
std::unique_ptr<Shape> SpaceShip::shoot()
{
	...
	auto bullet = std::make_unique<Shape>(Objects::cuboid(Vector3f{ shape_.heading()[0] + pos[0] + 0,
		shape_.heading()[1]+pos[1],shape_.heading()[2]+pos[2] }), Vector3f{ pos[0] + 0,pos[1] + -1,pos[0] });
